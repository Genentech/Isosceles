...
Isosceles can used in of the following run modes, specified when the `prepare_tcc_se` function is run:
  ...
  * **de_novo_loose** - de novo transcripts enabled, but all splice sites must be reproducibly passing filters in the aligned reads

# Genome annotations

Isosceles selects the appropriate set of transcripts to quantify, depending on the given run mode, using the genomic annotations that are prepared with the `prepare_transcripts` function. This function needs a GTF file with reference annotations, and, optionally, transcript structures extracted from BAM files using the `extract_read_structures` function (which is necessary for detecting de novo transcripts).
...
There are several important aspects of the `prepare_transcripts` function to consider:
  ...
  * Isosceles assigns new IDs to all transcripts (whether they are reference or de novo detected) based on stable hash identifiers. This simplifies the process of matching de novo transcripts across different data sets from the same genome build, irrespective of other experimental variables.  The corresponding reference ids (ENSG and ENST) compatible with each transcript can be found in rowData data `rowData(se)$compatible_tx`
  * Genes and transcripts are merged according to specific rules:
    ...
    * Annotated transcripts that share the same intron structure (or overlap on the same strand for unspliced transcripts), and have identical transcript start and end bins (default: 50 bp) are also merged, and given a unique transcript ID.
...
The comma separated list of compatible reference transcripts is in `rowData(se)`

# Output

The main output of Isosceles consists of SummarizedExperiment objects, created at various resolution levels:

  * **TCC (Transcript Compatibility Counts)** - This is an intermediate quantification stage of the data, formed directly from the BAM files with the `prepare_tcc_se` function. The values here are used to derive transcript quantification estimates via the EM algorithm. Both the `prepare_pseudobulk_se` function (for summarizing data at the pseudo-bulk level) and the `merge_sc_neighbors` function (for combining counts from neighboring cells) utilize TCC SummarizedExperiment objects as both input and output.
  * **transcript** - At the transcript level, expression values are computed using the `prepare_transcript_se` function, applying the EM (Expectation–Maximization) algorithm. By default, normalization is done using effective transcript lengths, but it can be turned off for UMI-based protocol data (commonly in scRNA-Seq datasets) by setting the *use_length_normalization* argument to FALSE. You can export transcript annotations from the SummarizedExperiment object to a GTF file using the `export_gtf` function.
  * **gene** - Gene-level expression values are derived using the `prepare_gene_se` function. This process is typically very swift as it doesn't necessitate running the EM algorithm.
  * **PSI (Percent Spliced In)** - PSI values are computed for unique transcriptomic regions of each gene (such as transcription start sites or core exons) using the `prepare_psi_se` function. This requires a transcript level SummarizedExperiment object as input.
...
* **rowData(se)** - additional data regarding the analyzed features (e.g. reference transcript ids or PSI regions)
