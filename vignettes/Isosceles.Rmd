...
Isosceles requires the following input files:
  ...
  * GTF file containing the genomic annotations
    ...
    * Isosceles was designed to work with GTF files originating from Ensembl / GENCODE (it uses some tags from these files by default). For datasets where such annotations are not available (e.g. SIRV data) the *is_technical* flag of the `prepare_transcripts` function can be set to TRUE in order to make Isosceles accept the GTF file
