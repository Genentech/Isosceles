...
their changes along a pseudotime trajectory. Given the number of PSI events 
found in annotations, we recommend filtering them (e.g. by the number of cells
showing signs of the event's inclusion) - in this vignette we'll restrict the 
analysis to the row names of the (already pre-filtered) `sce_psi` object. We 
can prepare a DEXSeqDataSet object for the glutamatergic trajectory 1 using the
following command:
```{r, message = FALSE, warning = FALSE}
dxd <- tcc_to_dexseq(
    se_tcc = se_tcc,
...
The object's design allows for an easy analysis of changes along pseudotime:
```{r, message = FALSE, warning = FALSE}
design(dxd)
```
